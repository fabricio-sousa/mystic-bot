...
==================================
A single-file, read-only monitoring dashboard for a bot.py instance running
in --shadow mode.

It reads the shadow_* files bot.py writes in shadow mode -- shadow_state.json,
shadow_trades.json, shadow_log.txt, shadow_heartbeat.txt, shadow_status.json --
from the same folder, and never touches the Kalshi API or your credentials.
It's the shadow counterpart to dashboard.py, which reads the unprefixed
live files (state.json etc.) -- run both side by side against the same
folder without either one touching the other's files.

Setup:
    pip install flask pytz
    Place this file in the same folder as bot.py and the shadow_* files it
    writes when run with --shadow.

Run:
    python shadow_dashboard.py
    python shadow_dashboard.py --port 5051   # if 5050 is taken, e.g. by
                                              # a second shadow variant

Then open http://localhost:5050 in a browser (default port -- deliberately
different from dashboard.py's 5000, so both can run at once on one
machine). The page polls for updates automatically -- no need to refresh.
"""
...
_argp.add_argument("--port", type=int, default=int(os.environ.get("PORT", 5050)),
                    help="Port to serve on (default: 5050, or $PORT if set)")
...
def api_status():
    ...
    # scanner (shadow_status.json) is written every loop and is the freshest
    # source for cash; shadow_state.json's shadow_cash is the fallback for
    # the moment right after bot.py starts, before the first loop has run.
    cash = (scanner or {}).get("cash", state.get("shadow_cash"))
    ...
    return jsonify({
        ...
        "scanner": scanner,
        "daily_drawdown_pct": (scanner or {}).get("daily_drawdown_pct"),
        "daily_drawdown_limit_pct": (scanner or {}).get("daily_drawdown_limit_pct"),
        "is_daily_paused": (scanner or {}).get("is_daily_paused", False),
        "daily_paused_until": (scanner or {}).get("daily_paused_until"),
...
<title>Mystic-Bot 1.0 [SHADOW]</title>
<style>
  :root {
    --bg:        #0a0b0d;
    --panel:     #111318;
  ...
  * { box-sizing: border-box; }
  ...
  .pause-badge {
    ...
    color: var(--accent);
    background: rgba(227, 163, 62, 0.12);
    border: 1px solid rgba(227, 163, 62, 0.35);
    padding: 3px 10px;
    border-radius: 3px;
  }

  .pause-badge.visible { display: inline-flex; }
...
<header>
  <div class="wordmark">MYSTIC<span>-</span>BOT<span> 1.0</span><span class="shadow-tag">SHADOW</span></div>
  <div class="status-pill">
    <span id="pause-badge" class="pause-badge">⏸ <span id="pause-text"></span></span>
...
<main>
  ...
  <div class="stats-row">
    <div class="stat">
      <div class="stat-label">Shadow Cash</div>
...
<footer>Mystic-Bot 1.0 Shadow Dashboard &middot; read-only &middot; polling every 3s</footer>
...
<script>
  ...
  // Drawdown is the inverse of pnl: positive % means cash is DOWN from
  // today's open (bad, red), negative % means cash is UP (good, green).
  const ddClass = (v) => (Number(v) > 0 ? "negative" : (Number(v) < 0 ? "positive" : ""));
  ...
  function renderScanner(scanner) {
    ...
    if (scanner.market) {
      ...
      const yesAskRow = (m.yes_ask !== undefined) ? `
          <div>
            <div class="field-label">Yes Ask</div>
            <div class="field-value ${m.yes_ask === entryThreshold ? "watch" : ""}">${m.yes_ask}&cent;</div>
          </div>` : "";
      const noAskRow = (m.no_ask !== undefined) ? `
          <div>
            <div class="field-label">No Ask</div>
            <div class="field-value ${m.no_ask === entryThreshold ? "watch" : ""}">${m.no_ask}&cent;</div>
          </div>` : "";
      marketBlock = `
        <div class="position-grid">
          <div>
  ...
  async function refresh() {
    try {
      ...
      if (data.daily_drawdown_pct === null || data.daily_drawdown_pct === undefined) {
        ...
        ddEl.textContent = (data.daily_drawdown_pct >= 0 ? "-" : "+") + Math.abs(data.daily_drawdown_pct).toFixed(1) + "%";
        ddEl.className = "stat-value " + ddClass(data.daily_drawdown_pct);
      }
      const pauseBadge = document.getElementById("pause-badge");
      if (data.is_daily_paused) {
        const until = data.daily_paused_until ? String(data.daily_paused_until).substring(11, 16) : "midnight";
        document.getElementById("pause-text").textContent = "DAILY DD PAUSE until " + until + " ET";
        pauseBadge.className = "pause-badge visible";
      } else {
